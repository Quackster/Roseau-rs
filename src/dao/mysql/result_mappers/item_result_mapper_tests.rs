...
fn maps_public_room_item_rows_using_room_id_and_definitions() {
    let definitions =
        ItemResultMapper::definitions(SqlExecutionResult::rows([definition_row(8, "chair", "")]))
            .unwrap();
    let result = SqlExecutionResult::rows([public_item_row(20, 8)]);
...
#[test]
fn skips_public_room_items_with_missing_definitions() {
    let definitions =
        ItemResultMapper::definitions(SqlExecutionResult::rows([definition_row(8, "chair", "")]))
            .unwrap();
    let result = SqlExecutionResult::rows([public_item_row(20, 8), public_item_row(21, 2415)]);

    let items = ItemResultMapper::public_room_items(result, 77, &definitions).unwrap();

    assert_eq!(items.len(), 1);
    assert!(items.contains_key(&20));
    assert!(!items.contains_key(&21));
}
