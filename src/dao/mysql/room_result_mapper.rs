...
impl RoomResultMapper {
    ...
    pub fn room_connections(result: SqlExecutionResult) -> Result<Vec<RoomConnection>, DaoError> {
        result.map_rows(|row| {
            ...
            Ok(room_connection_from_row(&connection_row))
        })
    }
...
mod tests {
    ...
    fn maps_ids_rights_connections_models_and_created_id() {
        ...
        let connections =
            RoomResultMapper::room_connections(SqlExecutionResult::rows([connection_row()]))
                .unwrap();
        ...
        let models =
            RoomResultMapper::room_models(SqlExecutionResult::rows([model_row()])).unwrap();
    ...
    fn rejects_wrong_result_kind_invalid_columns_and_large_insert_id() {
        ...
        assert_eq!(
            RoomResultMapper::created_room_id(SqlExecutionResult::insert_id(
                i64::from(i32::MAX) + 1
            ))
            .unwrap_err()
            .message(),
