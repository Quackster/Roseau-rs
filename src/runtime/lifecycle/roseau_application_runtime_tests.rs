...
fn public_room(id: i32, name: &str) -> RoomData {
    RoomData::new(
        id,
        false,
        RoomType::Public,
...
#[test]
fn loads_public_rooms_into_runtime_room_manager_in_dao_order() {
    let (root, bootstrap) = bootstrap_with_config("public-room-load", 0, 0);
    let binder = StdTcpSocketBinder::new();
    let mut application =
        RoseauApplicationRuntime::prepare(&bootstrap, &binder, [], 1, None).unwrap();

    application.load_public_rooms([public_room(5, "Habbo Lido"), public_room(4, "Club Massiva")]);

    let public_room_names = application
        .game()
        .room_manager()
        .get_public_rooms()
        .into_iter()
    ...
    assert_eq!(public_room_names, vec!["Habbo Lido", "Club Massiva"]);

    fs::remove_dir_all(root).unwrap();
}

#[test]
fn loads_catalogue_into_runtime_catalogue_manager() {
    let (root, bootstrap) = bootstrap_with_config("catalogue-load", 0, 0);
    let binder = StdTcpSocketBinder::new();
    let mut application =
        RoseauApplicationRuntime::prepare(&bootstrap, &binder, [], 1, None).unwrap();

    application.load_catalogue(
        [CatalogueItem::new("STN", 1174, 3)],
        [CatalogueDeal::new("OLYMPIA", ["STN"], 5)],
    );
