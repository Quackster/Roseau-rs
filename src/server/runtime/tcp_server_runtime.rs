...
impl TcpServerRuntime {
    ...
    pub fn accept_and_open_one<B: ServerSocketBinder>(
        ...
        runtime = runtime
            .with_context(IncomingContext::new().main_server_connection(listener_index == 0));
    ...
    pub fn drain_pending_logs(&mut self) -> Vec<String> {
        self.connections
            .iter_mut()
            .flat_map(TcpConnectionRuntime::drain_pending_logs)
            .collect()
    }
    ...
    pub fn drain_pending_incoming_commands(
        ...
        self.connections
            .iter_mut()
            .flat_map(TcpConnectionRuntime::drain_pending_incoming_commands)
            .collect()
    }
    ...
    pub fn step<B: ServerSocketBinder>(
        ...
        let accept_outcome = if accept_connection {
            match self.acceptor.accept_one_nonblocking(binder, listener_index) {
                Ok(Some(mut runtime)) => {
                    let connection_id = runtime.connection_id();
                    runtime = runtime.with_context(
                        IncomingContext::new().main_server_connection(listener_index == 0),
                    );
