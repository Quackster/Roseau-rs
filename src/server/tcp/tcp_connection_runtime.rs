...
use crate::server::{
    PlayerNetwork, PlayerNetworkEffect, PlayerNetworkEffectExecutor, ServerConnectionDriver,
...
pub struct TcpConnectionRuntime {
    ...
    emitted_connection_logs: usize,
    emitted_packet_logs: usize,
    emitted_sent_packet_logs: usize,
}
...
impl TcpConnectionRuntime {
    pub fn new(connection_id: i32, stream: TcpStream, remote_address: impl Into<String>) -> Self {
        Self {
            ...
            emitted_connection_logs: 0,
            emitted_packet_logs: 0,
            emitted_sent_packet_logs: 0,
        }
    }
    ...
    pub fn open(
        ...
        self.network_effect_executor
            .set_log_packets(connection_handler.log_packets());
        self.driver.open(
            server_handler,
            connection_handler,
    ...
    pub fn read_bytes(
        ...
        let result = self.driver.read_bytes(
            bytes,
            i32::from(self.network.server_port()),
            server_handler,
            connection_handler,
            &mut self.effect_executor,
        );
    ...
    pub fn drain_pending_incoming_commands(
        ...
        self.effect_executor.drain_pending_incoming_commands()
    }

    pub fn drain_pending_logs(&mut self) -> Vec<String> {
        let mut logs = Vec::new();
        logs.extend_from_slice(
            &self.effect_executor.connection_logs()[self.emitted_connection_logs..],
        );
        logs.extend_from_slice(&self.effect_executor.packet_logs()[self.emitted_packet_logs..]);
        logs.extend_from_slice(
            &self.network_effect_executor.packet_logs()[self.emitted_sent_packet_logs..],
        );
        self.emitted_connection_logs = self.effect_executor.connection_logs().len();
        self.emitted_packet_logs = self.effect_executor.packet_logs().len();
        self.emitted_sent_packet_logs = self.network_effect_executor.packet_logs().len();
        logs
    }
