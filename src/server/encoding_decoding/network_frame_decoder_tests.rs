...
#[test]
fn resynchronises_after_unframed_compatibility_bytes() {
    let mut decoder = NetworkFrameDecoder::new();

    let requests = decoder
        .push_bytes(b"figure=sd=001/0&hd=002/255,204,1530010GETCREDITS")
        .unwrap();

    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].get_header(), "GETCREDITS");
    assert_eq!(decoder.buffered_len(), 0);
}
