...
#[test]
fn plans_private_room_poster_order_info_from_catalogue_manager() {
    let manager = CatalogueManager::with_items_and_deals([CatalogueItem::new("PEN", 43, 2)], []);
    let catalogue = InMemoryCatalogueDao::new();
    let item_dao = Rc::new(InMemoryItemDao::new());
    let inventory = InMemoryInventoryDao::shared(Rc::clone(&item_dao));
    let buyer = buyer(25);
    ...
    assert_eq!(
        outcomes,
        vec![CatalogueIncomingOutcome::OrderInfo(
            CatalogueOrderInfoPlan::new("PEN", 2)
        )]
    );
}
