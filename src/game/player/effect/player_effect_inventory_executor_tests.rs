...
fn disposes_matching_player_inventory() {
    ...
    let applied = PlayerEffectInventoryExecutor::apply(
        &mut player,
...
#[test]
fn ignores_non_matching_and_non_inventory_player_effects() {
    let mut player = player();

    let count = PlayerEffectInventoryExecutor::apply_all(
        &mut player,
        &[
            PlayerEffect::DisposeInventory { user_id: 8 },
