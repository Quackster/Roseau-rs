...
mod tests {
    ...
    fn missing_user_maps_to_login_error_packet() {
        ...
        let outcome =
            PlayerLoginExecutor::login(&dao, &manager, request("unknown", "secret")).unwrap();
    ...
    fn wrong_password_maps_to_login_error_packet() {
        ...
        let outcome =
            PlayerLoginExecutor::login(&dao, &manager, request("alice", "wrong")).unwrap();
    ...
    fn correct_password_authenticates_with_raw_request_password() {
        ...
        let outcome =
            PlayerLoginExecutor::login(&dao, &manager, request("alice", "secret")).unwrap();
