    ...
    pub fn send_message(
        ...
        for receiver_id in receiver_ids {
            if messenger.is_friend(*receiver_id) {
                let message_id =
                    messenger_dao.new_message(messenger.user_id(), *receiver_id, message)?;
                deliveries.push(MessengerMessageDelivery::new(*receiver_id, message_id));
            }
        }
        ...
        if deliveries.is_empty() {
            ...
            Ok(MessengerCommandOutcome::MessagesSent { deliveries })
        }
    }
...
pub enum MessengerCommandOutcome {
    ...
    MessagesSent {
        deliveries: Vec<MessengerMessageDelivery>,
    },
    MessageMarkedRead {
        message_id: i32,
    },
...
pub struct MessengerMessageDelivery {
    receiver_id: i32,
    message_id: i32,
}

impl MessengerMessageDelivery {
    pub fn new(receiver_id: i32, message_id: i32) -> Self {
        Self {
            receiver_id,
            message_id,
        }
